...
class Prefs:
    ...
    sticker_price: int = 0
    # 价格概率（2026-09-08 用户需求）：{"0": 0.7, "10": 0.3}，key=价格
    # （>=10 一律映射「10 微信豆」档），value=权重，按比例抽样、和不必为 1。
    # 非空时每次发布一弹抽一次，覆盖固定价格；空 dict 回落 sticker_price。
    price_probs: dict = field(default_factory=dict)
